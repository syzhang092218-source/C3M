...
class InvertedPendulum(ControlAffineSystem):
    ...
    def render(self) -> np.ndarray:
        height = 5
        width = 5
        fig = plt.figure(figsize=(height, width), dpi=60)
        ...
        if self._action is not None:
            plt.text(text_point[0], text_point[1] - 3 * line_gap, f'U: {self._action.cpu().detach().numpy()}')
        ax.axis('off')
        canvas.draw()
    ...
    def render_demo(self, state: torch.Tensor, t: int, action: torch.Tensor = None) -> np.ndarray:
        ...
        if action is not None:
            plt.text(text_point[0], text_point[1] - 3 * line_gap, f'U: {action.cpu().detach().numpy()}')
        ax.axis('off')
        canvas.draw()
        data = np.frombuffer(canvas.tostring_rgb(), dtype='uint8').reshape(fig.canvas.get_width_height()[::-1] + (3,))
        plt.close()
